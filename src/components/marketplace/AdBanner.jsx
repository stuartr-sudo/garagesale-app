...
export default function AdBanner({ placement = "top_banner", className = "", onAdClick }) {
  ...
  if (placement === "sidebar") {
    return (
      <AdContainer ad={currentAd}>
        <div className="bg-slate-900 rounded-2xl p-4 shadow-lg border border-slate-700 cursor-pointer group hover:shadow-xl transition-all duration-300 relative">
  ...
  if (placement === "bottom_banner" || placement === "local_deals") {
    ...
    return (
      <AdContainer ad={currentAd}>
        <div className={`bg-slate-900 rounded-2xl shadow-lg transition-all duration-300 border border-slate-700 overflow-hidden cursor-pointer group hover:scale-[1.02] flex flex-col h-full relative ${
          isLocalDeal 
