-- Fixed Database Migration for Payment System
-- This handles the enum conversion properly
...
ALTER TABLE transactions ADD CONSTRAINT IF NOT EXISTS check_payment_method CHECK (payment_method IN ('bank_transfer', 'stripe', 'crypto'));
ALTER TABLE transactions ADD CONSTRAINT IF NOT EXISTS check_payment_status CHECK (payment_status IN ('pending', 'confirmed', 'completed', 'failed'));
